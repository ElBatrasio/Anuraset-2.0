...
# %% Set the working directory to the "anurabird" folder
script_dir = os.path.dirname(os.path.abspath(__file__))
# Move up one level to "anurabird"
...
labels_df = pd.read_csv(r'audio_species_mapping.csv', delimiter=',')
...
file_to_label = {
    row['audio_file']: [species for species in species_columns if row[species] == 1]
    for _, row in labels_df.iterrows()
}

# %% Load the JSON file
with open(r'data_test\embeddings\timestamps.json', 'r') as f:
...
for entry in timestamps:
    ...
    # Get the label for the audio file (assume labels are comma-separated for multi-label)
    label = file_to_label.get(audio_file, None)
    if label is not None:
        labels.append(label)  # Labels are already in list format
...
# %% Load embeddings
embeddings = np.load(
    r'data_test\embeddings\embeddings.npy')
...
model_save_folder = r'c:\Users\gbida\Projects\anurabird\models'
...
val_report_path = get_unique_path('validation_classification_report.csv')
...
test_report_path = get_unique_path('test_classification_report.csv')
...
np.save('embedding_min.npy', emb_min)
np.save('embedding_max.npy', emb_max)
